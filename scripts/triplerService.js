angular.module('triplerApp')
    .factory('triplerService', function ($http) {
        ...
        function parsePlaylistTracksFromAmrapResponse(xml) {
            var playlistTracks = [];
            xml.find(".playlist-track").children().not(".trackId").not(".noTrackContent").not(".trackContent").not(".trackShareUrl").each(function () {
